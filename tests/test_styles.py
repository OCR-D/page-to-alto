...
from ocrd_page_to_alto.styles import TextStylesManager, LayoutTagManager

def test_styles_id():
    m = TextStylesManager()
    assert m.get_id(font_family='Foo') == 'Foo---None---None---None---None---None'
    assert m.elements['Foo---None---None---None---None---None']['font_family'] == 'Foo'
...
def test_styles_to_xml():
    ...
    assert m.elements['Foo%20Serif---None---None---None---None---None']['font_family'] == 'Foo Serif'
...
def test_layouttagmanager():
    ...
    el = ET.Element('Tags')
    m.to_xml(el)
    assert ET.tostring(el) == b'<Tags><LayoutTag ID="paragraph" LABEL="paragraph"/></Tags>'
